...
public class IOTInverterCompany {
	public static void main(String[] args) {
		...
		PCU pci = new PCU(pciCurrent, pciVoltage); //Object of PCU
		display(pci);
		
		double gtiCurrent = 1.8;
		double gtiVoltage = 14.12;
		GTI gti = new GTI(gtiCurrent, gtiVoltage); //Object of GTI
		display(gti);
		
		double zelioCurrent = 1.4;
		double zelioVoltage = 13.21;
		Zelio zelio = new Zelio(zelioCurrent, zelioVoltage); //Object of Zelio
		display(zelio);
		
		double regaliaCurrent = 1.5;
		double regaliaVoltage = 11.21;
		Regalia regalia = new Regalia(regaliaCurrent, regaliaVoltage); //Object of Regalia
		display(regalia);
		
		double icruzeCurrent = 2.0;
		double icruzeVoltage = 17.1;
		ICruze icruze = new ICruze(icruzeCurrent, icruzeVoltage); //Object of iCruze
		display(icruze);
	}
